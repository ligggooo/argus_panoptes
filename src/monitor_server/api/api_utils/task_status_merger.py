...
class StatusMerger(StatusMergerInterface):
    def merge_info(self, records: List[StatusRecord]) -> Tuple[List[str], List[str]]:
        info = [r.desc for r in records if r.state!=ProcessState.failed and r.desc is not None and len(r.desc)]
        err = [r.desc for r in records if r.state == ProcessState.failed and r.desc is not None and len(r.desc)]
        return info, err

    def merge_status(self, records: List[StatusRecord]) -> Tuple[ProcessState, List[str], List[str], List[float]]:
        '''
